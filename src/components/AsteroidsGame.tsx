...
const AsteroidsGame = () => {
    ...
    useEffect(() => {
        ...
        const ASTEROID_SIZE = 50; // starting size
        const ASTEROID_SPEED = 50; // max starting speed
        const ASTEROID_VERT = 10; // avg number of vertices
        const ASTEROID_JAG = 0.4; // jaggedness
        const SHIP_INV_DUR = 3; // invincibility duration
        const SHIP_EXPLODE_DUR = 0.3; // duration of ship explosion
        const UFO_SPEED = 150;
        const UFO_SIZE = 20;
        const UFO_FIRE_RATE = 1.0; // seconds between shots
        const UFO_ACCURACY = 0.5; // accuracy of small ufo shots
        ...
        const destroyAsteroid = (index: number) => {
            ...
            if (score > 0 && score % 10000 < (score - (roid.r == Math.ceil(ASTEROID_SIZE) ? 20 : (roid.r == Math.ceil(ASTEROID_SIZE / 2) ? 50 : 100)))) {
                lives++;
            }
